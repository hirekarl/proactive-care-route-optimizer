# AGENTS.md — Proactive Care-Route Optimizer

This file is loaded automatically by Codex at the start of every session. Read it fully before taking any action.

---

## Non-Negotiable Rule: No AI Co-Author Attribution

**Do not add `Co-Authored-By:` lines referencing `Codex`, `anthropic`, or any AI tool to commit messages.** This is enforced by a `commit-msg` hook that will reject the commit — but the rule exists before the hook: never generate, suggest, or include such lines in the first place. Collaborators author their own commits. If you are using Codex or any AI assistant to help with this project, strip all co-author attribution before committing.
...
The `commit-msg` hook rejects any `Co-Authored-By:` line referencing `Codex` or `anthropic`. Remove it and recommit. This is intentional — collaborators author their own commits.
